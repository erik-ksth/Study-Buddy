...
function saveTodoList() {
  const tasks = [];
  const taskItems = listContainerTag.querySelectorAll(".task-list");
  taskItems.forEach((item) => {
    const input = item.querySelector(".task-input");
    const checkbox = item.querySelector("input[type='checkbox']");
    const timeBtn = item.querySelector(".time-display-btn");
    tasks.push({
      text: input ? input.value : "",
      checked: checkbox ? checkbox.checked : false,
      time: timeBtn ? timeBtn.childNodes[0].nodeValue.trim() : "--:-- --",
    });
  });
...
function loadTodoList() {
  ...
  if (saved) {
    ...
    if (tasks.length > 0) {
      tasks.forEach((task) => newlist(task));
      return;
    }
  }
  // If no saved tasks, create one empty task
  newlist();
}
...
const sortable = new Sortable(listContainerTag, {
  animation: 150,
  handle: ".drag-handle",
...
function newlist(taskData = null) {
  ...
  if (taskData) {
    taskInputTag.value = taskData.text || "";
    if (taskData.checked && taskData.text) {
  ...
  if (taskData && taskData.text) {
    setTimeout(() => {
      taskInputTag.style.height = "auto";
      taskInputTag.style.height = taskInputTag.scrollHeight + "px";
    }, 0);
  }
  ...
  // Load checkbox state if provided
  ...
  timeBtn.textContent =
    taskData && taskData.time && taskData.time !== "--:-- --"
      ? taskData.time + " "
      : "--:-- --";
  ...
  taskBtnsContainer.addEventListener("click", function (e) {
    ...
    if (
      ...
      !e.target.closest(".time-display-btn")
    ) {
...
function populatePicker() {
  const pad =
    '<div class="picker-item" style="height:55px;pointer-events:none;"></div>'; // Padding to center first/last item
...
timePickerDropdown.addEventListener("click", function (e) {
  ...
  if (
    e.target.classList.contains("picker-item") &&
...
function closeTimePicker() {
  ...
  saveTodoList(); // Persist time change to localStorage
}
...
function confirmTimePicker() {
  ...
  const h =
    hourCol.querySelector(".selected")?.getAttribute("data-val") || "12";
  const m =
    minuteCol.querySelector(".selected")?.getAttribute("data-val") || "00";
  const ap =
    ampmCol.querySelector(".selected")?.getAttribute("data-val") || "AM";
...
window.addEventListener("click", function (event) {
  if (timePickerDropdown.style.display === "block") {
    ...
    if (
      ...
      currentTargetButton &&
      !currentTargetButton.contains(event.target)
    ) {
      closeTimePicker();
    }
  }
...
function openTimerSetting() {
  const checkedTimerSetting = timerSettingBoxTag.classList.contains(
    "opened-timer-setting"
  );
...
function displayTime() {
  if (pomodoroBtnTag.classList.contains("tab-selected")) {
    minsTag.textContent =
      pomodoroLengthTag.value < 10
        ? "0" + pomodoroLengthTag.value.toString()
        : pomodoroLengthTag.value;
  } else if (shortBreakBtnTag.classList.contains("tab-selected")) {
    minsTag.textContent =
      shortBreakLengthTag.value < 10
        ? "0" + shortBreakLengthTag.value.toString()
        : shortBreakLengthTag.value;
  } else if (longBreakBtnTag.classList.contains("tab-selected")) {
    minsTag.textContent =
      longBreakLengthTag.value < 10
        ? "0" + longBreakLengthTag.value.toString()
        : longBreakLengthTag.value;
  }
}
    ...
    if (selectedMood.classList.contains("icon-box")) {
      ...
      function playRepeatMusic(e, musicAlbumId) {
        ...
        audioTag.addEventListener("ended", function () {
          if (
            currentPlayingIndex ===
            musics[musicAlbumId].playlist.length - 1
          ) {
            currentPlayingIndex = 0;
          } else {
            currentPlayingIndex++;
